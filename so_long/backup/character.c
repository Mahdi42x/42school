...
void move_player(int keycode) {
    if (keycode == 13 || keycode == 119 || keycode == 87) // 'W' oder 'w'
        player_y -= 10;
    else if (keycode == 0 || keycode == 97 || keycode == 65) // 'A' oder 'a'
        player_x -= 10;
    else if (keycode == 1 || keycode == 115 || keycode == 83) // 'S' oder 's'
        player_y += 10;
    else if (keycode == 2 || keycode == 100 || keycode == 68) // 'D' oder 'd'
        player_x += 10;

    // Bewegung mit Pfeiltasten
    else if (keycode == 123) // Pfeil nach links
        player_x -= 10;
    else if (keycode == 124) // Pfeil nach rechts
        player_x += 10;
    else if (keycode == 125) // Pfeil nach unten
        player_y += 10;
    else if (keycode == 126) // Pfeil nach oben
        player_y -= 10;
